...
namespace WarcraftPlugin.Classes
{
    internal class SacredWarrior : WarcraftClass
    {
        ...
        private void PlayerHurtOther(EventPlayerHurtOther @event)
        {
            ...
            if (level > 0)
            {
                // Apply burning effect to the victim
                var existing = WarcraftPlugin.Instance.EffectManager.GetEffectsByType<BurningSpearEffect>()
                    .Count(x => x.Victim.Handle == @event.Userid.Handle);
                if (existing < 3)
                {
